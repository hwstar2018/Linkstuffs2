...
public abstract class BaseUserServiceTest extends AbstractServiceTest {
    ...
    public void before() {
        ...
        tenantAdmin.setEmail("tenant@linkstuffs.com");
        ...
        customerUser.setEmail("customer@linkstuffs.com");
    ...
    public void testFindUserByEmail() {
        User user = userService.findUserByEmail(SYSTEM_TENANT_ID, "sysadmin@linkstuffs.com");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "tenant@linkstuffs.com");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "customer@linkstuffs.com");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "fake@linkstuffs.com");
    ...
    public void testSaveUser() {
        User tenantAdminUser = userService.findUserByEmail(SYSTEM_TENANT_ID,"tenant@linkstuffs.com");
        User user = new User();
        user.setAuthority(Authority.TENANT_ADMIN);
        user.setTenantId(tenantAdminUser.getTenantId());
        user.setEmail("tenant2@linkstuffs.com");
    ...
    public void testSaveUserWithSameEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@linkstuffs.com");
        tenantAdminUser.setEmail("sysadmin@linkstuffs.com");
    ...
    public void testSaveUserWithInvalidEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@linkstuffs.com");
        tenantAdminUser.setEmail("tenant@linkstuffs.com");
        userService.saveUser(tenantAdminUser);
    }
    ...
    public void testFindTenantAdmins() {
        ...
        for (int i = 0; i < 124; i++) {
            ...
            user.setEmail("testTenant" + i + "@linkstuffs.com");
    ...
    public void testFindTenantAdminsByEmail() {
        ...
        for (int i = 0; i < 94; i++) {
            ...
            String email = email1 + suffix + "@linkstuffs.com";
        ...
        for (int i = 0; i < 132; i++) {
            ...
            user.setTenantId(tenantId);
            String suffix = StringUtils.randomAlphanumeric((int) (5 + Math.random() * 10));
            String email = email2 + suffix + "@linkstuffs.com";
            email = i % 2 == 0 ? email.toLowerCase() : email.toUpperCase();
            user.setEmail(email);
    ...
    public void testFindCustomerUsers() {
        User customerUser = userService.findUserByEmail(tenantId, "customer@linkstuffs.com");
        ...
        for (int i = 0; i < 156; i++) {
            ...
            user.setEmail("testCustomer" + i + "@linkstuffs.com");
