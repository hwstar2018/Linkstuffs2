...
public class ThingsboardInstallService {
    ...
    public void performInstall() {
        try {
            if (isUpgrade) {
                log.info("Starting Linkstuffs Upgrade from version {} ...", upgradeFromVersion);

                cacheCleanupService.clearCache(upgradeFromVersion);

                if ("2.5.0-cassandra".equals(upgradeFromVersion)) {
                    log.info("Migrating Linkstuffs entities data from cassandra to SQL database ...");
                    entitiesMigrateService.migrate();
                    log.info("Updating system data...");
                    systemDataLoaderService.updateSystemWidgets();
                } else if ("3.0.1-cassandra".equals(upgradeFromVersion)) {
                    log.info("Migrating Linkstuffs latest timeseries data from cassandra to SQL database ...");
                    latestMigrateService.migrate();
                } else {
                    switch (upgradeFromVersion) {
                        case "1.2.3": //NOSONAR, Need to execute gradual upgrade starting from upgradeFromVersion
                            log.info("Upgrading Linkstuffs from version 1.2.3 to 1.3.0 ...");
                        ...
                        case "1.3.0":  //NOSONAR, Need to execute gradual upgrade starting from upgradeFromVersion
                            log.info("Upgrading Linkstuffs from version 1.3.0 to 1.3.1 ...");
                        ...
                        case "1.3.1": //NOSONAR, Need to execute gradual upgrade starting from upgradeFromVersion
                            log.info("Upgrading Linkstuffs from version 1.3.1 to 1.4.0 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("1.3.1");

                        case "1.4.0":
                            log.info("Upgrading Linkstuffs from version 1.4.0 to 2.0.0 ...");
                        ...
                        case "2.0.0":
                            log.info("Upgrading Linkstuffs from version 2.0.0 to 2.1.1 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.0.0");

                        case "2.1.1":
                            log.info("Upgrading Linkstuffs from version 2.1.1 to 2.1.2 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.1.1");
                        case "2.1.3":
                            log.info("Upgrading Linkstuffs from version 2.1.3 to 2.2.0 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.1.3");

                        case "2.3.0":
                            log.info("Upgrading Linkstuffs from version 2.3.0 to 2.3.1 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.3.0");

                        case "2.3.1":
                            log.info("Upgrading Linkstuffs from version 2.3.1 to 2.4.0 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.3.1");

                        case "2.4.0":
                            log.info("Upgrading Linkstuffs from version 2.4.0 to 2.4.1 ...");

                        case "2.4.1":
                            log.info("Upgrading Linkstuffs from version 2.4.1 to 2.4.2 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.4.1");
                        case "2.4.2":
                            log.info("Upgrading Linkstuffs from version 2.4.2 to 2.4.3 ...");

                            databaseEntitiesUpgradeService.upgradeDatabase("2.4.2");

                        case "2.4.3":
                            log.info("Upgrading Linkstuffs from version 2.4.3 to 2.5 ...");
                        ...
                        case "2.5.0":
                            log.info("Upgrading Linkstuffs from version 2.5.0 to 2.5.1 ...");
                            if (databaseTsUpgradeService != null) {
                                databaseTsUpgradeService.upgradeDatabase("2.5.0");
                            }
                        case "2.5.1":
                            log.info("Upgrading Linkstuffs from version 2.5.1 to 3.0.0 ...");
                        case "3.0.1":
                            log.info("Upgrading Linkstuffs from version 3.0.1 to 3.1.0 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.0.1");
                            dataUpdateService.updateData("3.0.1");
                        case "3.1.0":
                            log.info("Upgrading Linkstuffs from version 3.1.0 to 3.1.1 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.1.0");
                        case "3.1.1":
                            log.info("Upgrading Linkstuffs from version 3.1.1 to 3.2.0 ...");
                            if (databaseTsUpgradeService != null) {
                        ...
                        case "3.2.0":
                            log.info("Upgrading Linkstuffs from version 3.2.0 to 3.2.1 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.2.0");
                        case "3.2.1":
                            log.info("Upgrading Linkstuffs from version 3.2.1 to 3.2.2 ...");
                            if (databaseTsUpgradeService != null) {
                                databaseTsUpgradeService.upgradeDatabase("3.2.1");
                            }
                            databaseEntitiesUpgradeService.upgradeDatabase("3.2.1");
                        case "3.2.2":
                            log.info("Upgrading Linkstuffs from version 3.2.2 to 3.3.0 ...");
                            if (databaseTsUpgradeService != null) {
                            ...
                            systemDataLoaderService.createOAuth2Templates();
                        case "3.3.0":
                            log.info("Upgrading Linkstuffs from version 3.3.0 to 3.3.1 ...");
                        case "3.3.1":
                            log.info("Upgrading Linkstuffs from version 3.3.1 to 3.3.2 ...");
                        case "3.3.2":
                            log.info("Upgrading Linkstuffs from version 3.3.2 to 3.3.3 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.3.2");
                            dataUpdateService.updateData("3.3.2");
                        case "3.3.3":
                            log.info("Upgrading Linkstuffs from version 3.3.3 to 3.3.4 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.3.3");
                        case "3.3.4":
                            log.info("Upgrading Linkstuffs from version 3.3.4 to 3.4.0 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.3.4");
                            dataUpdateService.updateData("3.3.4");
                        case "3.4.0":
                            log.info("Upgrading Linkstuffs from version 3.4.0 to 3.4.1 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.4.0");
                            dataUpdateService.updateData("3.4.0");
                        case "3.4.1":
                            log.info("Upgrading Linkstuffs from version 3.4.1 to 3.4.2 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.4.1");
                            dataUpdateService.updateData("3.4.1");
                            log.info("Updating system data...");
                            systemDataLoaderService.updateSystemWidgets();
                        case "3.4.2":
                            log.info("Upgrading Linkstuffs from version 3.4.2 to 3.4.3 ...");
                        case "3.4.3":
                            log.info("Upgrading Linkstuffs from version 3.4.3 to 3.4.4 ...");
                            break;

                        //TODO update CacheCleanupService on the next version upgrade

                        default:
                            throw new RuntimeException("Unable to upgrade Linkstuffs, unsupported fromVersion: " + upgradeFromVersion);
                ...
                log.info("Starting Linkstuffs Installation...");
...
//                systemDataLoaderService.loadSystemRules();
            ...
            log.error("Unexpected error during Linkstuffs installation!", e);
            throw new ThingsboardInstallException("Unexpected error during Linkstuffs installation!", e);
        } finally {
